...
interface StudentCountMap {
  [key: string]: number;
}

export function StudentGroupPage() {
  const [studentGroups, setStudentGroups] = useState<StudentGroup[]>([]);
  const [studentCountMap, setStudentCountMap] = useState<StudentCountMap>({});

  const [studentGroupEditorModalState, setStudentGroupEditorModalState] =
    useState<StudentGroupEditorModalState>({
      studentGroupId: null,
      isOpen: false,
    });
  const [
    removeStudentGroupConfirmModalState,
    setRemoveStudentGroupConfirmModalState,
  ] = useState<RemoveStudentGroupConfirmModalState>({
    studentGroupId: null,
  ...
  useEffect(() => {
    ...
    loadStudentCountOnGroups(groupIds);
  }, [studentGroups]);
  ...
  async function loadStudentCountOnGroups(groupIds: string[]) {
    try {
      const studentCountList =
        await StudentProvider.GetStudentsCountOnGroupIds(groupIds);
      if (studentCountList == null) return;
      const byStudentId = studentCountList.reduce<StudentCountMap>((acc, x) => {
        acc[x.groupId] = x.countStudents;
        return acc;
      }, {});

      setStudentCountMap((prev) => ({ ...prev, ...byStudentId }));
    } catch (e) {
      const message = e instanceof Error ? e.message : "Unknown error";
      setErrorMessage(message);
    }
  ...
  function openStudentGroupEditorModal(studentGroupId?: string) {
    setStudentGroupEditorModalState({
  ...
  function openRemoveStudentGroupConfirmModal(
    ...
    setRemoveStudentGroupConfirmModalState({
      ...
      ...ConfirmModalState.getOpen(
        `Вы действительно хотите удалить студенческую группу "${studentGroupName}"`,
      ),
    });
  }
  ...
  async function closeRemoveStudentGroupConfirmModal(isConfirmed: boolean) {
    if (isConfirmed) {
      if (removeStudentGroupConfirmModalState.studentGroupId == null)
        throw "Cannot remove student group with StudentGroupId = null";

      const result = await StudentGroupProvider.markStudentGroupAsRemoved(
        removeStudentGroupConfirmModalState.studentGroupId,
      );
      if (!result.isSuccess) {
    ...
    >
      ...
      >
        ...
        <Button
          variant="add"
          title="Создать"
          onClick={() => openStudentGroupEditorModal()}
        />
      </Paper>
              ...
              {studentGroups.map((studentGroup) => {
                return (
                  <TableRow key={`studentGroup__${studentGroup.id}`}>
                    ...
                    <TableCell width="20%">
                      {studentGroup.abbreviation}
                    </TableCell>
                    <TableCell width="15%">
                      {studentGroup.studyStartYear} -{" "}
                      {studentGroup.studyEndYear}
                    </TableCell>
                    <TableCell width="15%">{getCourse(studentGroup)}</TableCell>
                    <TableCell width="15%">
                      {TrainingFormatUtils.getDisplayName(
                        studentGroup.trainingFormat,
                      )}
                    </TableCell>
                    <TableCell width="15%">
                      {studentCountMap[studentGroup.id]}
                    </TableCell>
                    <TableCell>
                      <Button
                        type="icon"
                        ...
                        size="small"
                        onClick={() =>
                          openStudentGroupEditorModal(studentGroup.id)
                        }
                      />
                      <Button
                        type="icon"
                        ...
                        size="small"
                        onClick={() =>
                          openRemoveStudentGroupConfirmModal(
                            studentGroup.id,
                            studentGroup.name,
                          )
                        }
      ...
      <ConfirmModal
        ...
        onClose={(isConfirmed) =>
          closeRemoveStudentGroupConfirmModal(isConfirmed)
        }
